...
logbook.dta %>% filter(codesp==10119) %>% 
  ...
  group_by(nation) %>% 
  summarize(total=sum(poids)) %>% 
  bind_rows(request1)->request2

  
ggplot(request1)+geom_bar(aes(x=nation,y=total),stat='identity')+
   coord_flip()


ggplot(request1)+geom_bar(aes(x=reorder(nation,total),y=total),stat='identity')+
   coord_flip()
...
maturite.dta %>% mutate(lt=10*floor(lt/100),classe_taille=paste(lt,'-',lt+10,sep='')) %>% #Passage des longueur en classe de taille de 10 cm ( lt=20)
  ...
  pivot_wider(names_from=nom,values_from=pct) %>% # Le tableau croisé est là On ne cite ici que la champ dont les valeurs 

...
vignette: >
  %\VignetteIndexEntry{Day 1 Pablo}
...
library(AquaticLifeHistory) # Package pour les modèles de croissance
```
...
Cette équation est utilisée en écologie et en gestion des pêches pour modéliser la longueur moyenne en fonction de l'âge chez les animaux (notamment des poissons et d'autres organismes marins). En ajustant les paramètres Linf, k, et L0 à des données empiriques, les scientifiques peuvent prédire la taille future des populations et mieux comprendre les dynamiques de croissance dans divers environnements. 
...
# Fitting specific models of your choice
Estimate_Growth(data = growth_data, models = "VB")
Estimate_Growth(data = growth_data, models = c("Log", "Gom"))

results <- Estimate_Growth(data = growth_data, plots = FALSE) ### Stockage des résultats
Length_at_age_estimates <- results$Estimates
head(Length_at_age_estimates) ### Avoir une idée de la taille pour les différents âges

```

Linf est la longueur asymptotique, c'est-à-dire la longueur maximale que l'organisme peut atteindre.
K est le coefficient de croissance qui détermine la rapidité avec laquelle l'organisme atteint Linf
L0 est l'âge hypothétique auquel la longueur de l'organisme serait nulle (c'est souvent un ajustement pour tenir compte de la phase initiale de croissance).

Le dernier tableau permet d'avoir une idée de la taille attendue pour un âge donné.

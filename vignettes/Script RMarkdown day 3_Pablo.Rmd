...
Data_Fish_rec <- data("Data_Fish_rec.dta") # Vient du package formationIMROP
```
...
```{r}
Rec_temp <- ggplot(Data_Fish_rec, aes(x = Annee, y = Recrue)) +
...
SSB_temp <- ggplot(Data_Fish_rec, aes(x = Annee, y = Biomasse.feconde)) +
  geom_line(color = "blue") +      # Add a line
  geom_point(color = "blue", size = 3) + # Add points
  labs(title = "",
       x = "Annee",
...
SST_temp <- ggplot(Data_Fish_rec, aes(x = Annee, y = SST)) +
  geom_line(color = "blue") +      # Add a line
  geom_point(color = "blue", size = 3) + # Add points
  labs(title = "",
       x = "Annee",
...
Phyto_temp <- ggplot(Data_Fish_rec, aes(x = Annee, y = Phytoplancton)) +
  geom_line(color = "blue") +      # Add a line
  geom_point(color = "blue", size = 3) + # Add points
  labs(title = "",
       x = "Annee",
...
Zoo_temp <- ggplot(Data_Fish_rec, aes(x = Annee, y = Zooplancton)) +
  geom_line(color = "blue") +      # Add a line
  geom_point(color = "blue", size = 3) + # Add points
  labs(title = "",
       x = "Annee",
...
WeMOi_temp <- ggplot(Data_Fish_rec, aes(x = Annee, y = WeMOi)) +
  geom_line(color = "blue") +      # Add a line
  geom_point(color = "blue", size = 3) + # Add points
  labs(title = "",
       x = "Annee",
...
#  Faire un GAM pour étudier les liens potentiels entre l'environnement et le recrutement de l'anchois

Vérifier s'il n'y a pas de multicolinéarité

```{r}
model <- lm(Recrue ~ Biomasse.feconde + SST + Phytoplancton +Zooplancton + WeMOi, data = Data_Fish_rec)
library(car)
...
```{r}
gam_anchois_rec <- gam(Recrue ~ s(Biomasse.feconde,k=3) + s(SST,k=3)+s(Phytoplancton,k=3)+s(Zooplancton,k=3) + s(WeMOi,k=3), data = Data_Fish_rec,
                   family = Gamma(link = "identity"), 
                   method = "REML")
summary(gam_anchois_rec)
plot(gam_anchois_rec, page=1, se = TRUE, shade = TRUE)
options(na.action = "na.fail")
dredge(gam_anchois_rec)
